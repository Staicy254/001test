...
int my_print_alias(data_of_program *data, char *alias)
{
    ...
    if (data->alias_list)
    {
        ...
        {
            if (!alias || (strncmp(data->alias_list[q], alias, alias_length) == 0
                && data->alias_list[q][alias_length] == '='))
            {
                ...
                buffer[r + 1] = '\0';
                strncat(buffer, "'", 1);
                strncat(buffer, data->alias_list[q] + r + 1, sizeof(buffer) - strlen(buffer) - 1);
                strncat(buffer, "'\n", 2);
                _print(buffer);
            }
        }
...
char *my_get_alias(data_of_program *data, char *name)
{
    ...
    /* argument validation */
    if (name == NULL || name[0] == '\0' || data->alias_list == NULL)
        return (NULL);

    alias_length = my_strlen(name);

    for (q = 0; data->alias_list[q]; q++)
    {
        /* checking for coincidence in varname; through iteration of the environ */
        if (my_strcmp(name, data->alias_list[q], alias_length) &&
            data->alias_list[q][alias_length] == '=')
        {
...
int my_set_alias(char *alias_string, data_of_program *data)
{
    ...
    for (r = 0; data->alias_list[r]; r++)
        if (my_strcmp(buffer, data->alias_list[r], q) &&
            data->alias_list[r][q] == '=')
        {
    ...
    if (temp)
    {
        ...
        data->alias_list[r] = strdup(buffer);
    }
    else
    {
        /* if added alias does not exist */
        data->alias_list[r] = strdup(alias_string);
    }
    return (0);
}